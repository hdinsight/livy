...
object TestEnvConfig {
  private val properties = {
    ...
    val testEnvPropStream = getClass.getClassLoader.getResourceAsStream("test-env.properties")
    assert(testEnvPropStream != null, "Cannot find test-env.properties in CLASSPATH.")
    p.load(testEnvPropStream)
    p.asScala
  }
...
class RealClusterPool extends ClusterPool {
  ...
  assert(ipList.length > 0,
    "RealCluster pool is empty. Please specify cluster ip in test-env.properties")
